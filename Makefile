...
test: verifiers
	@echo "Running all coverage for admin-on-rest-server"
	@./buildscripts/go-coverage.sh

build:
	@echo "Building admin-on-rest-server to $(PWD)/admin-on-rest-server ..."
	# @CGO_ENABLED=1 go build -o $(PWD)/admin-on-rest-server github.com/checkr/flagr/swagger_gen/cmd/admin-on-rest-server

run:
	@$(PWD)/admin-on-rest-server --port 18000
